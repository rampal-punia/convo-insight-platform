...
class TrackingRequest(BaseModel):
    ...
    customer_id: str = Field(description="The ID of the customer making the request")
...
class TrackingDetailRequest(TrackingRequest):
    ...
    include_history: bool = Field(
        default=False, description="Whether to include full tracking history"
    )
...
class TrackingUpdateRequest(TrackingRequest):
    ...
    notify: bool = Field(
        default=True, description="Whether to enable tracking notifications"
    )
...
class SummarizeConversation(BaseModel):
    ...
    max_length: int = Field(
        default=150, description="Maximum length of the summary in words"
    )
...
class ToolRegistry:
    ...
    def is_sensitive_tool(self, tool_name: str) -> bool:
        ...
        is_sensitive = (
            metadata is not None and metadata.category == ToolCategory.SENSITIVE
        )
...
class ToolManager:
    ...
    def _create_tracking_details_tool(self):
        ...
        async def get_tracking_details(
            order_id: str, customer_id: str, include_history: bool = False
        ) -> str:
    ...
    def _format_tracking_details(self, tracking_info: Dict) -> str:
        ...
        if "history" in tracking_info:
            ...
            for event in tracking_info["history"]:
                details += (
                    f"- {event['timestamp']}: {event['status']} - {event['location']}\n"
                )
    ...
    def _create_modify_order_tool(self):
        ...
        async def modify_order_quantity(
            order_id: str, customer_id: str, product_id: int, new_quantity: int
        ) -> str:
            """Modify the quantity of a product in an order"""
            # First validate order status
            try:
                can_modify, message = (
                    await self.db_ops.validate_order_status_for_modification(order_id)
                )
                if not can_modify:
    ...
    def _create_cancel_order_tool(self):
        ...
        async def cancel_order(order_id: str, customer_id: str, reason: str) -> str:
            ...
            try:
                result = await self.db_ops.update_order(
                    order_id, {"action": "cancel_order", "reason": reason}
                )
                return result["message"]
            except Exception as e:
