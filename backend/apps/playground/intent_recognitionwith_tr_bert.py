    ...
    class ImprovedIntentClassifier(nn.Module):
        def __init__(self, base_model_name, num_categories, num_intents):
            ...
            self.category_projection = nn.Sequential(
                nn.Linear(hidden_size, 128), nn.ReLU(), nn.Linear(128, 64)
            )
        ...
        def forward(self, input_ids=None, attention_mask=None, labels=None, **kwargs):
            ...
            category_logits = F.cosine_similarity(
                category_projected.unsqueeze(1), category_embeddings.unsqueeze(0), dim=2
            )

            intent_logits = F.cosine_similarity(
                intent_projected.unsqueeze(1), intent_embeddings.unsqueeze(0), dim=2
            )
...
class IntentModelTester:
    ...
    def predict(self, text: str) -> dict:
        ...
        reverse_intent_mapping = {
            v: k for k, v in self.mappings["intent_mapping"].items()
        }
